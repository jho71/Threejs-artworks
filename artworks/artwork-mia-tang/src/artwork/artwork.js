  ...
  useEffect(() => {
    ...
    function initSceneHelper() {
      const gridHelper = new GridHelper(
        constants.options.grid.size,
        constants.options.grid.divisions
      );
    ...
    function initGround() {
      ...
      let box = createStaticBox(
        scene,
        constants.groundInfo.size,
        constants.groundInfo.pos,
        [0, 0, 0],
        0xffffff
      );
      grounds.push(box);
    ...
    function initWalls() {
      ...
      let sideBoxRight = createStaticBox(
        scene,
        sideBoxSize,
        leftBoxPos,
        [0, 0, 0],
      ...
      let sideBoxLeft = createStaticBox(
        scene,
        sideBoxSize,
        rightBoxPos,
        [0, 0, 0],
      ...
      let sideBoxTop = createStaticBox(
        scene,
        horizBoxSize,
        topBoxPos,
        [0, 0, 0],
      ...
      let sideBoxBot = createStaticBox(
        scene,
        horizBoxSize,
        botBoxPos,
        [0, 0, 0],
    ...
    function checkCollision() {
      ...
      for (let set = 0; set < NUM_SETS; set++) {
        ...
        if (meshes.some(fns.hasTween)) {
          ...
          for (let i = 0; i < meshes.length - 1; i++) {
            ...
            if (aTween || bTween) {
              ...
              rSum =
                circA.geometry.parameters.radius +
                circB.geometry.parameters.radius;
        ...
        if (
          toggles[set].animated &&
          !toggles[set].hasDescartes &&
          allCollided
        ) {
          if (DEBUG) {
